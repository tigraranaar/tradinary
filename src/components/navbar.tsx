...
export default function Navbar() {
    ...
    return (
        <>
                                ...
                                <AnimatePresence>
                                    {showUserMenu && (
                                        <motion.div
                                            ...
                                            exit={{ opacity: 0, y: -10 }}
                                            className="absolute right-0 mt-2 w-48 glass rounded-lg p-2 border border-white/20"
                                        >
                                            <button
                                                onClick={handleSignOut}
                                                className="w-full text-left px-3 py-2 text-sm hover:bg-white/10 rounded transition cursor-pointer"
                                            >
                                                Sign Out
                                            </button>
                                        </motion.div>
            ...
            <div className={`fixed inset-0 z-50 flex flex-col items-center justify-center gap-6 bg-black/20 text-lg font-medium backdrop-blur-2xl transition duration-300 md:hidden ${isOpen ? 'translate-x-0' : '-translate-x-full'}`} suppressHydrationWarning>
                {!loading && user && links.map((link) => (
